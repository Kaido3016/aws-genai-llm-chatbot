...
export interface ChatBotHistoryItem {
  ...
  metadata: Record<
    ...
    | ChatBotFilesBlob
  >;
  // A3: safe, redacted citations — populated for ALL authenticated users
  // (unlike `metadata`, which stays admin/workspace_manager-only). Never
  // assume this is present; older messages or non-RAG turns may omit it.
  citations?: Citation[];
  tokens?: ChatBotToken[];
