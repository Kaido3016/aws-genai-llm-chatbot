...
import { ChatMessageCitations } from "./chat-message-citations";
    ...
    <div style={{ marginTop: message.type == "ai" ? "0" : "0.5em" }}>
      ...
      >
        <>
          ...
          {/* A3: user-facing "Sources" — shown to every authenticated
              user, independent of the admin-only debug metadata panel
              above (`expandableContent`). Only rendered once the answer
              has finished streaming, and only for AI messages. */}
          {!loading && props.message?.type === ChatBotMessageType.AI && (
            <ChatMessageCitations citations={props.message.citations} />
          )}
