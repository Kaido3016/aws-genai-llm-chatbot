    ...
    def get_qa_prompt(self, custom_prompt=None):
        if custom_prompt:
            qa_system_prompt_with_context = (
                custom_prompt
                + "\n\n"
                + prompt_guard.UNTRUSTED_CONTEXT_PREAMBLE
                + "\n\n{context}"
            )
        else:
            ...
            #
            # A2.1: the retrieved-document text substituted into
            # {context} has already been delimited and had forged
            # delimiters neutralized by SanitizingWorkspaceRetriever
            # (see genai_core/langchain/workspace_retriever.py). The
            # preamble below is the second, complementary layer: it
            # tells the model explicitly, in its own system prompt, how
            # to treat that delimited content. Reusing
            # prompt_guard.UNTRUSTED_CONTEXT_PREAMBLE (rather than a new,
            # separately-worded string here) keeps the wording that
            # describes the <source> boundary in one place.
            qa_system_prompt_with_context = (
                qa_system_prompt
                + "\n\n"
                + prompt_guard.UNTRUSTED_CONTEXT_PREAMBLE
